...
use hasher::ArkworksPoseidonHasherBn254;
...
impl Poseidon {
    ...
    pub fn hash(&self, inputs: Vec<&[u8]>) -> Result<[u8; 32]> {
        ...
        let hash_result = match num_inputs {
            2 => {
                ArkworksPoseidonHasherBn254::hash(&packed_inputs, &self.hasher_params_width_3_bytes)
            }
            3 => {
                ArkworksPoseidonHasherBn254::hash(&packed_inputs, &self.hasher_params_width_4_bytes)
            }
            4 => {
                ArkworksPoseidonHasherBn254::hash(&packed_inputs, &self.hasher_params_width_5_bytes)
            }
