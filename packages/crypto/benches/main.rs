...
// Test groth16 verify for bn254, bls381 is the same
const PUBLIC_INPUTS:&str="b7a08d5962f1dc5778f9f2385c291ce6f76f9b075e028e6500f327203bebc61f5219025541ad12054aaf6c0ee8c5ae09f23b693068a9cc80fd960f666cd65121e0fb95195eb23c3c65d7df44a7adf83871ec0380189d3e757417765d51ae0d2a";
...
fn bench_crypto(c: &mut Criterion) {
    ...
    group.bench_function("keccak_256", |b| {
        let message = hex::decode(COSMOS_SECP256K1_MSG_HEX).unwrap();
        b.iter(|| {
    ...
    group.bench_function("curve_hash_bls381", |b| {
        let message = hex::decode(COSMOS_SECP256K1_MSG_HEX).unwrap();
        let message_hash = Sha256::digest(&message);
        b.iter(|| {
            assert!(!curve_hash(&message_hash, 0).is_empty());
        });
    });

    group.bench_function("curve_hash_bn254", |b| {
        let message = hex::decode(COSMOS_SECP256K1_MSG_HEX).unwrap();
        let message_hash = Sha256::digest(&message);
        b.iter(|| {
            assert!(!curve_hash(&message_hash, 1).is_empty());
        });
    });

    group.bench_function("poseidon_hash_bls381", |b| {
    ...
    group.bench_function("poseidon_hash_bn254", |b| {
        let commitment_hash = hex::decode(COMMITMENT).unwrap();
        let poseidon = Poseidon::new();
        b.iter(|| {
    ...
    group.bench_function("groth16_verify_bn254", |b| {
        let input = hex::decode(PUBLIC_INPUTS).unwrap();
        let proof = hex::decode(PROOF).unwrap();
        let vk = hex::decode(VK).unwrap();
        b.iter(|| {
            assert!(groth16_verify(&input, &proof, &vk, 1).unwrap());
        });
    });
