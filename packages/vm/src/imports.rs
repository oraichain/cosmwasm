...
/// Extends api
pub fn do_groth16_verify<A: BackendApi + 'static, S: Storage + 'static, Q: Querier + 'static>(
    mut env: FunctionEnvMut<Environment<A, S, Q>>,
    input_ptr: u32,
    proof_ptr: u32,
    vk_ptr: u32,
    ...
    let input = read_region(
        ...
        input_ptr,
        MESSAGE_HASH_MAX_LEN * 3,
    )?;
    let proof = read_region(&data.memory(&mut store), proof_ptr, GROTH16_PROOF_LEN)?;
    let vk = read_region(&data.memory(&mut store), vk_ptr, GROTH16_VERIFIER_KEY_LEN)?;
    ...
    let gas_info = GasInfo::with_cost(data.gas_config.groth16_verify_cost);
    process_gas_info(data, &mut store, gas_info)?;
...
pub fn do_poseidon_hash<A: BackendApi + 'static, S: Storage + 'static, Q: Querier + 'static>(
    mut env: FunctionEnvMut<Environment<A, S, Q>>,
    left_input_ptr: u32,
    right_input_ptr: u32,
    curve: u32,
    ...
    let left_input = read_region(
        &data.memory(&mut store),
        left_input_ptr,
        MESSAGE_HASH_MAX_LEN * 2,
    )?;
    let right_input = read_region(
        &data.memory(&mut store),
        right_input_ptr,
        MESSAGE_HASH_MAX_LEN * 2,
    )?;
    ...
    let gas_info = GasInfo::with_cost(data.gas_config.poseidon_hash_cost);
    process_gas_info(data, &mut store, gas_info)?;
    let result = data.poseidon.hash(&left_input, &right_input, curve);

    match result {
        Ok(hash) => {
            write_region(&data.memory(&mut store), hash_ptr, &hash)?;
            Ok(0)
        }
        Err(_) => Err(VmError::generic_err("poseidon hash error".to_string())),
    }
}

pub fn do_curve_hash<A: BackendApi + 'static, S: Storage + 'static, Q: Querier + 'static>(
    mut env: FunctionEnvMut<Environment<A, S, Q>>,
    input_ptr: u32,
    curve: u32,
    hash_ptr: u32,
    ...
    let gas_info = GasInfo::with_cost(data.gas_config.curve_hash_cost);
    process_gas_info(data, &mut store, gas_info)?;
...
pub fn do_keccak_256<A: BackendApi + 'static, S: Storage + 'static, Q: Querier + 'static>(
    mut env: FunctionEnvMut<Environment<A, S, Q>>,
    input_ptr: u32,
    hash_ptr: u32,
) -> VmResult<u32> {
    ...
    let gas_info = GasInfo::with_cost(data.gas_config.keccak_256_cost);
    process_gas_info(data, &mut store, gas_info)?;
...
pub fn do_sha256<A: BackendApi + 'static, S: Storage + 'static, Q: Querier + 'static>(
    mut env: FunctionEnvMut<Environment<A, S, Q>>,
    input_ptr: u32,
    hash_ptr: u32,
) -> VmResult<u32> {
    ...
    let gas_info = GasInfo::with_cost(data.gas_config.sha256_cost);
    process_gas_info(data, &mut store, gas_info)?;
...
pub fn do_db_next<A: BackendApi + 'static, S: Storage + 'static, Q: Querier + 'static>(
    ...
    let out_data = if data.interface_version == 4 {
...
mod tests {
    ...
    fn make_instance(
        ...
        let env = Environment::new(api, gas_limit, None);
