    ...
    Q: Querier + 'static, // 'static is needed here to allow using this in an Environment that is cloned into closures
{
    ...
    pub(crate) fn from_module(
        ...
        env_imports.insert(
            "groth16_verify",
            Function::new_typed_with_env(&mut store, &fe, do_groth16_verify),
        );

        // poseidon hash
        env_imports.insert(
            "poseidon_hash",
            Function::new_typed_with_env(&mut store, &fe, do_poseidon_hash),
        );

        // keccak 256
        env_imports.insert(
            "keccak_256",
            Function::new_typed_with_env(&mut store, &fe, do_keccak_256),
        );

        // sha256
        env_imports.insert(
            "sha256",
            Function::new_typed_with_env(&mut store, &fe, do_sha256),
        );

        // curve hash
        env_imports.insert(
            "curve_hash",
            Function::new_typed_with_env(&mut store, &fe, do_curve_hash),
        );
    ...
    pub(crate) fn get_interface_version(&self) -> u8 {
        self.fe.as_ref(&self.store).interface_version
    }
}
